...
// Neither bionic, the iOS SDK nor newlib implements the *context() functions,
// so these use their own AArch64 context switch. iOS must be tested before the
// MAC branch below, which it also satisfies.
#include <cstdint>
#include <vector>
...
#elif REX_PLATFORM_ANDROID || REX_PLATFORM_IOS || REX_PLATFORM_SWITCH
