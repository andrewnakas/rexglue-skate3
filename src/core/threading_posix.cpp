...
// Robust mutexes let a wait recover when the owning thread dies (EOWNERDEAD).
// Bionic implements neither PTHREAD_MUTEX_ROBUST nor pthread_mutex_consistent,
// so Android falls back to plain std::mutex - the same path macOS uses.
